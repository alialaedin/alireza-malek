<x-dashboard::admin.layout.master id="app">
	...
	<div class="page-header">
		<x-breadcrumb :items="config('admin-breadcrumbs.companies.edit')" />
	</div>

	<x-card title="ویرایش اطلاعات شرکت">
		<div class="row">
			...
			<div class="col-xl-3">
				<x-form-group>
					...
					<multiselect dir="rtl" class="custom-multiselect form-control-sm p-0" v-model="company.activity_license"
						label="label" placeholder="انتخاب مجوز فعالیت" track-by="name" :options="activityLicenses"
		...
		<script>
			...
			createApp({
				...
				mounted() {
					...
					this.company.national_code = company.national_code;
					this.company.brand = company.brand;
					this.company.username = company.username;
					this.company.address = company.address;
					this.company.login_status = company.login_status ? true : false;
					this.company.workshop_code = company.workshop_code;
					this.company.signature_owners = company.signature_owners;
					this.company.type = this.types.find(t => t.name == company.type);
					this.company.activity_license = this.activityLicenses.find(a => a.name == company.activity_license);

					this.company.signature_owners?.forEach(signatureOwner => {
						signatureOwner.has_right_to_sign = signatureOwner.has_right_to_sign ? true : false;
					});
					...
					async submit() {
						...
						data.signature_owners = data.signature_owners?.map(s => {
							...
							return {
								...
								has_right_to_sign: s.has_right_to_sign,
							};
						});
						...
						const url = @json(route('admin.companies.update', $company));
						const options = {
							method: 'PATCH',
							headers: {
