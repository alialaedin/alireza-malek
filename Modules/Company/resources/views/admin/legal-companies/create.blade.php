<x-dashboard::admin.layout.master id="app">
	...
	<div class="page-header">
		<x-breadcrumb :items="config('admin-breadcrumbs.legal-companies.create')" />
	</div>
	...
	<x-card title="اطلاعات شرکت حقوقی">
		<div class="row">
			...
			<div class="col-12">
				<x-form-group>
					<x-checkbox name="login_status" title="وضعیت لاگین" :is-checked="1" v-model="company.login_status" />
				</x-form-group>
			</div>
			...
			createApp({
				...
				methods: {
					...
					async submit() {
						...
						this.company['signature_owners'] = this.signatureOwners ?? [];

						const url = @json(route('admin.legal-companies.store'));
						const options = {
							method: 'POST',
							headers: {
