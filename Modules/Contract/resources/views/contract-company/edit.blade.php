<x-dashboard::admin.layout.master>
  ...
  <x-card title="ویرایش قرارداد">
    <x-form :action="route('admin.contract-companies.update', $contractCompany)" method="PATCH" enctype="multipart/form-data">
      <div class="row">
        ...
        <div class="col-xl-3">
          <x-form-group>
            ...
            <input type="text" readonly class="form-control fs-12"
                   value="{{ $contractCompany->company->getName() . ' - ' . $contractCompany->company->getMobile() }}"/>
          </x-form-group>
        </div>
          ...
          <x-form-group>
            ...
            <select name="status" id="status" class="form-control fs-12">
              ...
              @foreach ($statuses as $status)
                <option
                  value="{{ $status['name'] }}" @selected(old('status', $contractCompany->status->value) == $status['name'])>
                  {{ $status['label'] }}
                </option>
              @endforeach
            </select>
